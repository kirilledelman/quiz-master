...
// new - returns null
// edit - returns quiz data for editing the quiz (only owner can edit)
// view - returns quiz data for taking the quiz (if not published, only owner can view)
export async function quizLoader({ request, params }) {
	console.log("quizLoader", request, params );
	// new quiz
...
export default function QuizEditPage() {
	...
	function publishedChanged(event){
		quiz.published = event.target.checked;
		setQuiz({...quiz});
		setDirty();
	...
	async function saveQuiz() {
		...
		if ( quiz.published && !quiz.data.questions.find(q => ['choice','multiple'].includes(q.type))) validation.published = "Can't publish quiz with no questions";
		// check each question
	...
	<form action={formActionFunc} className="box">
		...
		<InputField name="published" label="Published" reverseLabel type="checkbox" value={quiz.published} error={validation.published} onChange={publishedChanged}/>
		...
		<ErrorMessage>{validation.form}</ErrorMessage>
